...
class NumberSeries {
  ...
  init(_ input: String) throws {
    guard input.allSatisfy(\.isNumber) else { throw NumberSeriesError.invalidCharacter }
    self.input = input
  }
  ...
  func largestProduct(_ span: Int) throws -> Int {
    ...
    guard span <= count else { throw NumberSeriesError.spanLongerThanInput }
    ...
    for end in 0..<count {
      ...
      guard let num = digit.wholeNumberValue else { continue }
      ...
      if end - start + 1 == span {
        ...
        let startDigit = input[startIndex]
        guard let startNum = startDigit.wholeNumberValue else { continue }
